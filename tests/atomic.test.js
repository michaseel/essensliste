...
forAll(config.getDevices(), function (device) {
  ...
  test('Footer on ' + device.deviceName, function () { gl.openPage(device, config.getProjectPage()+'/3_organisms/footer/footer.html'); gl.runSpecFile(device, 'app/3_organisms/footer/footer.spec', device.tags);});
  test('Header on ' + device.deviceName, function () { gl.openPage(device, config.getProjectPage()+'/3_organisms/header/header.html'); gl.runSpecFile(device, 'app/3_organisms/header/header.spec', device.tags);});
  // [endinjector]
});
